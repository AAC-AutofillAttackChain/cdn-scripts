/* thirdparty_cdn.js
   CDN-style third-party script：動態注入一組與 loginForm.html 相同的 20 種隱藏技術
   可放在不同 origin (e.g. 127.0.0.1:8081) 提供，載入即 append 到 body 前端（或指定 target）。
*/
(function(){
  const TECHS = [
  ...
  container.id = 'thirdparty-cdn-injected';
  container.setAttribute('data-injected-by','thirdparty-cdn');
  ...
  h.textContent = '=== thirdparty_cdn: injected hidden-tech test set ===';
  ...
  TECHS.forEach(t => {
    ...
    if (t.wrapperHtml) {
      ...
      summary.textContent = 'thirdparty_cdn (detail) summary';
    ...
    if (t.shadowHost) {
      ...
      const host = document.createElement('x-shadow-host-thirdparty-cdn');
      host.id = `${t.id}-shadow-cdn`;
    ...
    revealBtn.textContent = 'reveal (thirdparty_cdn)';
    ...
    revealBtn.addEventListener('click', () => {
      ...
      const host = card.querySelector('x-shadow-host-thirdparty-cdn');
  ...
  console.log('[thirdparty_cdn] injected hidden-tech test set');
})();
